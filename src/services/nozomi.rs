//! Nozomi / temporal fast-landing relay stub.
...
pub async fn send_transaction(tx: &VersionedTransaction) -> Result<String> {
    let url = env::var("NOZOMI_API_URL").map_err(|_| anyhow!("NOZOMI_API_URL is not set"))?;
    let api_key = env::var("NOZOMI_API_KEY").unwrap_or_default();
    ...
    body.get("signature")
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .ok_or_else(|| anyhow!("nozomi response missing signature: {body}"))
}
